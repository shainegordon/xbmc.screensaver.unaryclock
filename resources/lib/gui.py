...
class Screensaver(xbmcgui.WindowXMLDialog):
    ...
    def showClock(self, onlySeconds):
	    ...
	    self.topX = random.randint(1, maxX)
            ...
            self.drawSinglePart(0, 0, (hour/10), blockSizeNormal, 'red.png', 0, self.randHourMin)
            self.drawSinglePart(3*(self.lightSizeNormal+self.lightPaddingNormal)+1*self.blockPaddingSmall, 0, (hour%10), blockSizeNormal, 'cyan.png', 9, self.randHourMin)
       
            minute = now.minute
            #self.log(('minute ' + str(minute)))
            self.drawSinglePart(6*(self.lightSizeNormal+self.lightPaddingNormal)+1*self.blockPaddingSmall+self.blockPaddingLarge, 0, (minute/10), blockSizeNormal, 'green.png', 18, self.randHourMin)
            self.drawSinglePart(9*(self.lightSizeNormal+self.lightPaddingNormal)+2*self.blockPaddingSmall+self.blockPaddingLarge, 0, (minute%10), blockSizeNormal, 'purple.png', 27, self.randHourMin)
        if (self.showSeconds == True):
           second = now.second
           self.drawSinglePart(12*(self.lightSizeNormal+self.lightPaddingNormal)+3*self.blockPaddingSmall+2*self.blockPaddingLarge, 0, second, blockSizeSeconds, 'blue.png', 36, self.randSec)
