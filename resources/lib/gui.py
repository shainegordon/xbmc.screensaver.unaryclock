...
image_dir = xbmc.translatePath(
    os.path.join(addon_path, 'resources', 'skins', 'default', 'media', '').encode("utf-8"))
...
class Screensaver(xbmcgui.WindowXMLDialog):
    ...
    def drawSinglePart(self, xOffset, yOffset, numberOfLights, blockSize, texture, imageOffset, randomized):
        ...
        if (blockSize > blockSizeNormal):
            ...
            lightPadding = 3
        lightSize = round(lightSize)
        for cell in range(0, blockSize * blockSize):
            column = cell % blockSize
            row = cell / blockSize
            t = 'grey.png'

            newX = round(self.topX + xOffset + column * (lightSize + lightPadding))
            newY = round(self.topY + yOffset + row * (lightSize + lightPadding))
    ...
    def showClock(self, onlySeconds):
        ...
        if (onlySeconds == False):
            ...
            self.totalClockWidth = 4 * (blockSizeNormal * (
                        self.lightSizeNormal + self.lightPaddingNormal)) + 2 * self.blockPaddingSmall + 1 * self.blockPaddingLarge
            if (self.showSeconds):
                self.totalClockWidth = self.totalClockWidth + (blockSizeNormal * (
                            self.lightSizeNormal + self.lightPaddingNormal)) + 1 * self.blockPaddingLarge
            ...
            self.topX = random.randint(1, round(maxX))
            self.topY = random.randint(1, round(maxY))

            hour = now.hour
            # self.log(('hour ' + str(hour)))
            self.drawSinglePart(0, 0, math.floor(hour / 10), blockSizeNormal, self.hour1Color, 0, self.randHourMin)
            self.drawSinglePart(3 * (self.lightSizeNormal + self.lightPaddingNormal) + 1 * self.blockPaddingSmall, 0,
                                (hour % 10), blockSizeNormal, self.hour2Color, 9, self.randHourMin)

            minute = now.minute
            # self.log(('minute ' + str(minute)))
            self.drawSinglePart(6 * (
                        self.lightSizeNormal + self.lightPaddingNormal) + 1 * self.blockPaddingSmall + self.blockPaddingLarge,
                                0, math.floor(minute / 10), blockSizeNormal, self.min1Color, 18, self.randHourMin)
            self.drawSinglePart(9 * (
                        self.lightSizeNormal + self.lightPaddingNormal) + 2 * self.blockPaddingSmall + self.blockPaddingLarge,
                                0, (minute % 10), blockSizeNormal, self.min2Color, 27, self.randHourMin)
        if (self.showSeconds == True):
            second = now.second
            self.drawSinglePart(12 * (
                        self.lightSizeNormal + self.lightPaddingNormal) + 3 * self.blockPaddingSmall + 2 * self.blockPaddingLarge,
